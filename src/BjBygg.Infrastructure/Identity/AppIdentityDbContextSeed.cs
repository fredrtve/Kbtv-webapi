...
namespace BjBygg.Infrastructure.Identity
{
    public class AppIdentityDbContextSeed
    {
            ...
            AuthSettings authSettings)
        {
            ...
            if (userManager.FindByNameAsync(Roles.Admin).Result == null)
            {
                var user = new ApplicationUser { UserName = Roles.Admin, SecurityStamp = Guid.NewGuid().ToString(), FirstName = "Admin", LastName = "Admin" };
                var result = await userManager.CreateAsync(user, authSettings.AdminPassword);
